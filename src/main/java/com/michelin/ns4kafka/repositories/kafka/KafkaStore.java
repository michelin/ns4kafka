...
public abstract class KafkaStore<T> {
    ...
    private void createOrVerifyInternalTopic() throws KafkaStoreException {
        try {
            ...
            throw new KafkaStoreException("Thread interrupted trying to create or validate configuration of topic " + kafkaTopic + ".", e);
        } catch (ExecutionException e) {
            throw new KafkaStoreException("Execution error trying to create or validate configuration of topic " + kafkaTopic + ".", e);
        } catch (TimeoutException e) {
            throw new KafkaStoreException("Timed out trying to create or validate configuration of topic " + kafkaTopic + ".", e);
        }
    }
     ...
     */
    private void verifyInternalTopic() throws KafkaStoreException, InterruptedException, ExecutionException, TimeoutException {
        ...
        if (!TopicConfig.CLEANUP_POLICY_COMPACT.equals(retentionPolicy)) {
            if (log.isErrorEnabled()) {
                log.error("The retention policy of the topic " + kafkaTopic + " is incorrect. "
                        ...
                        + "deleting your data after a week. Refer to Kafka documentation for more details on cleanup policies");
            }
     ...
     */
    private void createInternalTopic() throws KafkaStoreException, InterruptedException, ExecutionException, TimeoutException {
    ...
    private long getLatestOffset() throws KafkaStoreException {
        ...
        try {
            ...
            Thread.currentThread().interrupt();
            throw new KafkaStoreException("Thread interrupted while waiting for the latest offset of topic " + kafkaTopic + ".", e);
        } catch (ExecutionException e) {
            throw new KafkaStoreException("Execution error while waiting for the latest offset of topic " + kafkaTopic + ".", e);
        } catch (TimeoutException e) {
            throw new KafkaStoreException("Timeout while waiting for the latest offset of topic " + kafkaTopic + ".", e);
        } catch (Exception e) {
        ...
        try {
            ...
            while ((offsetInSchemasTopic < offset) && (timeoutNs > 0)) {
                try {
                    ...
                    Thread.currentThread().interrupt();
                    log.debug("Interrupted while waiting for the background store reader thread to reach the specified offset: {}",
                            offset, e);
                }
            }
        } finally {
