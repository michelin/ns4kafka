...
public class KafkaStreamRepository extends KafkaStore<KafkaStream> implements StreamRepository {
    ...
    public KafkaStreamRepository(
            ...
            @Value("${ns4kafka.store.kafka.topics.prefix}." + InternalTopic.STREAM) String kafkaTopic,
    ...
    public List<KafkaStream> findAllForCluster(String cluster) {
        List<KafkaStream> results = new ArrayList<>();
        try (KeyValueIterator<String, KafkaStream> iterator = queryAllStore()) {
    ...
    public void create(KafkaStream stream) {
        produce(getMessageKey(stream), stream);
    }
