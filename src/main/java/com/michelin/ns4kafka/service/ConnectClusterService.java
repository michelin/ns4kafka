...
public class ConnectClusterService {
    ...
    private static final Set<AccessControlEntry.Permission> OWNER_AND_WRITE_PERMISSIONS =
    ...
    public Flux<ConnectCluster> findAll(boolean all, boolean status) {
        Flux<ConnectCluster> selfHostedConnectClusters =
                Flux.defer(() -> Flux.fromIterable(connectClusterRepository.findAll().stream()
    ...
    public ConnectCluster create(ConnectCluster connectCluster) {
        ...
        connectCluster.getMetadata().setGeneration(connectCluster.getMetadata().getGeneration() + 1);
    ...
    public List<VaultResponse> vaultPassword(
        ...
        final Optional<ConnectCluster> kafkaConnect =
                findAllForNamespaceByPermissions(namespace, OWNER_AND_WRITE_PERMISSIONS).stream()
