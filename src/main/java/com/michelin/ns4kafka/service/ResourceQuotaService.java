...
public class ResourceQuotaService {
    ...
    public void create(ResourceQuota resourceQuota) {
        resourceQuotaRepository.create(resourceQuota);
    }
    ...
    public List<String> validateNewResourceQuota(Namespace namespace, ResourceQuota resourceQuota) {
        ...
        if (StringUtils.hasText(resourceQuota.getSpec().get(COUNT_TOPICS.getKey()))) {
            ...
            if (used > limit) {
                errors.add(invalidQuotaAlreadyExceeded(COUNT_TOPICS, String.valueOf(limit), String.valueOf(used)));
            }
        }
        ...
        if (StringUtils.hasText(resourceQuota.getSpec().get(COUNT_PARTITIONS.getKey()))) {
            ...
            if (used > limit) {
                errors.add(invalidQuotaAlreadyExceeded(COUNT_PARTITIONS, String.valueOf(limit), String.valueOf(used)));
            }
        }
        ...
        if (StringUtils.hasText(resourceQuota.getSpec().get(DISK_TOPICS.getKey()))) {
            ...
            if (!limitAsString.endsWith(BYTE)
                    && !limitAsString.endsWith(KIBIBYTE)
                    && !limitAsString.endsWith(MEBIBYTE)
                    && !limitAsString.endsWith(GIBIBYTE)) {
                ...
                if (used > limit) {
                    errors.add(invalidQuotaAlreadyExceeded(
                            DISK_TOPICS, limitAsString, BytesUtils.bytesToHumanReadable(used)));
                }
            }
        }
        ...
        if (StringUtils.hasText(resourceQuota.getSpec().get(COUNT_CONNECTORS.getKey()))) {
            ...
            if (used > limit) {
                errors.add(invalidQuotaAlreadyExceeded(COUNT_CONNECTORS, String.valueOf(limit), String.valueOf(used)));
            }
        }
    ...
    public long getCurrentCountPartitionsByNamespace(Namespace namespace) {
        return topicService.findAllForNamespace(namespace).stream()
        ...
        if (existingTopic.isEmpty()) {
            ...
            if (StringUtils.hasText(resourceQuota.getSpec().get(COUNT_PARTITIONS.getKey()))) {
                ...
                if (used + newTopic.getSpec().getPartitions() > limit) {
                    errors.add(invalidQuotaOperationCannotAdd(
                            COUNT_PARTITIONS,
                            String.valueOf(used),
        ...
        if (StringUtils.hasText(resourceQuota.getSpec().get(DISK_TOPICS.getKey()))
            ...
            if (bytesToAdd > 0 && used + bytesToAdd > limit) {
                errors.add(invalidQuotaOperationCannotAdd(
                        ...
                        BytesUtils.bytesToHumanReadable(limit),
                        BytesUtils.bytesToHumanReadable(bytesToAdd)));
            }
        }
     ...
     * @return A list of quotas as response format
     */
    public ResourceQuotaResponse getUsedResourcesByQuotaByNamespace(
            Namespace namespace, Optional<ResourceQuota> resourceQuota) {
        ...
        return formatUsedResourceByQuotaResponse(
                namespace,
                currentCountTopic,
     ...
     * @return A list of quotas as response format
     */
    public ResourceQuotaResponse formatUsedResourceByQuotaResponse(
            Namespace namespace,
            long currentCountTopic,
            long currentCountPartition,
            long currentDiskTopic,
